...
from electrum_spero.i18n import _, languages
from electrum_spero.util import FileImportFailed, FileExportFailed, make_aiohttp_session, resource_path
from electrum_spero.invoices import PR_UNPAID, PR_PAID, PR_EXPIRED, PR_INFLIGHT, PR_UNKNOWN, PR_FAILED, PR_ROUTING, PR_UNCONFIRMED
...
def export_meta_gui(electrum_window: 'ElectrumWindow', title, exporter):
    ...
    filename = getSaveFileName(
        ...
        filename='electrum-spero_{}.json'.format(title),
        filter=filter_,
        config=electrum_window.config,
    )
    if not filename:
