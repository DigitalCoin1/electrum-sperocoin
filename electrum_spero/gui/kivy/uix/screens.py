...
from electrum_spero.util import profiler, parse_URI, format_time, InvalidPassword, NotEnoughFunds, Fiat
from electrum_spero.invoices import (PR_TYPE_ONCHAIN, PR_TYPE_LN, PR_DEFAULT_EXPIRATION_WHEN_CREATING,
                                   PR_PAID, PR_UNKNOWN, PR_EXPIRED, PR_INFLIGHT,
                                   LNInvoice, pr_expiration_values, Invoice, OnchainInvoice)
from electrum_spero import bitcoin, constants
from electrum_spero.transaction import Transaction, tx_from_any, PartialTransaction, PartialTxOutput
from electrum_spero.util import parse_URI, InvalidBitcoinURI, TxMinedInfo, maybe_extract_bolt11_invoice
from electrum_spero.wallet import InternalAddressCorruption
from electrum_spero import simple_config
from electrum_spero.lnaddr import lndecode
from electrum_spero.lnutil import RECEIVED, SENT, PaymentFailure
from electrum_spero.logging import Logger

from .dialogs.question import Question
from .dialogs.confirm_tx_dialog import ConfirmTxDialog

from electrum_spero.gui.kivy import KIVY_GUI_PATH
from electrum_spero.gui.kivy.i18n import _

if TYPE_CHECKING:
    from electrum_spero.gui.kivy.main_window import ElectrumWindow
...
class HistoryScreen(CScreen):
    ...
    def get_card(self, tx_item): #tx_hash, tx_mined_status, value, balance):
        ...
        if is_lightning:
            ...
            fee_text = '' if fee is None else 'fee: %d spr'%fee
        else:
...
class SendScreen(CScreen, Logger):
    ...
    def read_invoice(self):
        ...
        if not address:
            self.app.show_error(_('Recipient not specified.') + ' ' + _('Please scan a SperoCoin address or a payment request'))
            return
        if not self.amount:
        ...
        if self.is_lightning:
            ...
            if self.payment_request:
                ...
                if not bitcoin.is_address(address):
                    self.app.show_error(_('Invalid SperoCoin Address') + ':\n' + address)
                    return
...
class ReceiveScreen(CScreen):
    ...
    def get_URI(self):
        from electrum_spero.util import create_bip21_uri
        amount = self.amount
        if amount:
