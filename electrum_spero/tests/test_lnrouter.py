...
from electrum_spero.util import bh2u, bfh, create_and_start_event_loop
from electrum_spero.lnutil import ShortChannelID
from electrum_spero.lnspero import (SperoHopsDataSingle, new_spero_packet,
                                  process_spero_packet, _decode_spero_error, decode_spero_error,
                                  SperoFailureCode, SperoPacket)
from electrum_spero import bitcoin, lnrouter
from electrum_spero.constants import BitcoinTestnet
from electrum_spero.simple_config import SimpleConfig
from electrum_spero.lnrouter import PathEdge, LiquidityHintMgr, DEFAULT_PENALTY_PROPORTIONAL_MILLIONTH, DEFAULT_PENALTY_BASE_MSAT, fee_for_edge_msat
...
class Test_LNRouter(TestCaseForTestnet):
    ...
    def test_new_spero_packet_legacy(self):
        ...
        hops_data = [
            SperoHopsDataSingle(is_tlv_payload=False, payload={
        ...
        packet = new_spero_packet(payment_path_pubkeys, session_key, hops_data, associated_data)
    ...
    @needs_test_with_all_chacha20_implementations
    def test_new_spero_packet_mixed_payloads(self):
        # test vector from bolt-04
        payment_path_pubkeys = [
            bfh('02eec7245d6b7d2ccb30380bfbe2a3648cd7a942653f5aa340edcea1f283686619'),
            bfh('0324653eac434488002cc06bbfb7f10fe18991e35f9fe4302dbea6d2353dc0ab1c'),
        ...
        hops_data = [
            ...
            SperoHopsDataSingle(is_tlv_payload=True),
            SperoHopsDataSingle(is_tlv_payload=True),
            SperoHopsDataSingle(is_tlv_payload=True),
            SperoHopsDataSingle(is_tlv_payload=False, payload={
                "amt_to_forward": {"amt_to_forward": 4},
    ...
    @needs_test_with_all_chacha20_implementations
    def test_process_spero_packet_mixed_payloads(self):
        ...
        hops_data = [
            ...
            SperoHopsDataSingle(is_tlv_payload=True, payload={
                "amt_to_forward": {"amt_to_forward": 1},
                "outgoing_cltv_value": {"outgoing_cltv_value": 1},
                "short_channel_id": {"short_channel_id": bfh('0101010101010101')},
            }),
        ...
        for i, privkey in enumerate(payment_path_privkeys):
            processed_packet = process_spero_packet(packet, associated_data, privkey)
            self.assertEqual(hops_data[i].to_bytes(), processed_packet.hop_data.to_bytes())
            packet = processed_packet.next_packet

    @needs_test_with_all_chacha20_implementations
    def test_process_spero_packet_legacy(self):
        # this test is not from bolt-04, but is based on the one there;
    ...
    def test_decode_spero_error(self):
        ...
        decoded_error, index_of_sender = _decode_spero_error(error_packet_for_node_0, payment_path_pubkeys, session_key)
        ...
        failure_msg, index_of_sender = decode_spero_error(error_packet_for_node_0, payment_path_pubkeys, session_key)
        self.assertEqual(4, index_of_sender)
        self.assertEqual(SperoFailureCode.TEMPORARY_NODE_FAILURE, failure_msg.code)
