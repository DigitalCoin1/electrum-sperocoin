...
from electrum_spero import bitcoin
from electrum_spero import constants
from electrum_spero.network import Network
from electrum_spero.ecc import ECPrivkey
from electrum_spero import simple_config, lnutil
from electrum_spero.lnaddr import lnencode, LnAddr, lndecode
from electrum_spero.bitcoin import COIN, sha256
from electrum_spero.util import bh2u, create_and_start_event_loop, NetworkRetryManager, bfh
from electrum_spero.lnpeer import Peer, UpfrontShutdownScriptViolation
from electrum_spero.lnutil import LNPeerAddr, Keypair, privkey_to_pubkey
from electrum_spero.lnutil import LightningPeerConnectionClosed, RemoteMisbehaving
from electrum_spero.lnutil import PaymentFailure, LnFeatures, HTLCOwner
from electrum_spero.lnchannel import ChannelState, PeerState, Channel
from electrum_spero.lnrouter import LNPathFinder, PathEdge, LNPathInconsistent
from electrum_spero.channel_db import ChannelDB
from electrum_spero.lnworker import LNWallet, NoPathFound
from electrum_spero.lnmsg import encode_msg, decode_msg
from electrum_spero import lnmsg
from electrum_spero.logging import console_stderr_handler, Logger
from electrum_spero.lnworker import PaymentInfo, RECEIVED
from electrum_spero.lnspero import SperoFailureCode
from electrum_spero.lnutil import derive_payment_secret_from_payment_preimage
from electrum_spero.lnutil import LOCAL, REMOTE
from electrum_spero.invoices import PR_PAID, PR_UNPAID
...
class TestPeer(TestCaseForTestnet):
    ...
    def test_payment_multihop_temp_node_failure(self):
        ...
        async def pay(lnaddr, pay_req):
            ...
            self.assertEqual(PR_UNPAID, graph.w_d.get_payment_status(lnaddr.paymenthash))
            self.assertEqual(SperoFailureCode.TEMPORARY_NODE_FAILURE, log[0].failure_msg.code)
            raise PaymentDone()
        async def f():
            async with TaskGroup() as group:
                for peer in peers:
    ...
    def test_payment_with_temp_channel_failure_and_liquidty_hints(self):
        ...
        async def pay(lnaddr, pay_req):
            ...
            self.assertEqual(2, len(log))
            self.assertEqual(PR_PAID, graph.w_d.get_payment_status(lnaddr.paymenthash))
            self.assertEqual(SperoFailureCode.TEMPORARY_CHANNEL_FAILURE, log[0].failure_msg.code)
