...
class SimpleConfig(Logger):
    ...
    def get_wallet_path(self, *, use_gui_last_wallet=False):
        ...
        old_path = os.path.join(self.path, "electrum-spero.dat")
    ...
    def get_fee_text(
        ...
        if fee_per_kb is None:
            ...
            rate_str = format_fee_satoshis(fee_per_byte) + ' spr/byte'
    ...
    def format_fee_rate(self, fee_rate):
        return format_fee_satoshis(fee_rate/1000, num_zeros=self.num_zeros) + ' spr/byte'
...
def read_user_config(path):
    """Parse and store the user config settings in electrum-spero.conf into user_config[]."""
    if not path:
        return {}
