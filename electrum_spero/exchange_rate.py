...
class BitcoinVenezuela(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        ...
        rates = [(r, json['SPERO'][r]) for r in json['SPERO']
                 if json['SPERO'][r] is not None]  # Giving NULL sometimes
        return dict(rates)
    ...
    async def request_history(self, ccy):
        json = await self.get_json('api.bitcoinvenezuela.com',
                             "/historical/index.php?coin=SPERO")
...
class CoinCap(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        json = await self.get_json('api.coincap.io', '/v2/rates/SperoCoin/')
    ...
    async def request_history(self, ccy):
        ...
        history = await self.get_json('api.coincap.io',
                                      '/v2/assets/SperoCoin/history?interval=d1&limit=2000')
...
class CoinGecko(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        json = await self.get_json('api.coingecko.com', '/api/v3/coins/sperocoin')
    ...
    async def request_history(self, ccy):
        history = await self.get_json('api.coingecko.com',
                                      '/api/v3/coins/SperoCoin/market_chart?vs_currency=%s&days=max' % ccy)
