...
;Variables
  ...
  !define PRODUCT_WEB_SITE "https://github.com/pooler/electrum-spero"
...
;General
  ...
  ;Name and file
  Name "${PRODUCT_NAME}"
  OutFile "dist/electrum-spero-setup.exe"
...
;Interface Settings
  ...
  !define MUI_ICON "c:\electrum-spero\electrum_spero\gui\icons\electrum.ico"
...
Section
  ...
  File /r "dist\electrum-spero\*.*"
  File "c:\electrum-spero\electrum_spero\gui\icons\electrum.ico"
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-spero-${PRODUCT_VERSION}.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-spero-${PRODUCT_VERSION}.exe" "" "$INSTDIR\electrum-spero-${PRODUCT_VERSION}.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME} Testnet.lnk" "$INSTDIR\electrum-spero-${PRODUCT_VERSION}.exe" "--testnet" "$INSTDIR\electrum-spero-${PRODUCT_VERSION}.exe" 0


  ;Links bitcoin: URI's to Electrum
  WriteRegStr HKCU "Software\Classes\SperoCoin" "" "URL:SperoCoin Protocol"
  WriteRegStr HKCU "Software\Classes\SperoCoin" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\SperoCoin" "DefaultIcon" "$\"$INSTDIR\electrum.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\SperoCoin\shell\open\command" "" "$\"$INSTDIR\electrum-spero-${PRODUCT_VERSION}.exe$\" $\"%1$\""
...
Section "Uninstall"
  ...
  DeleteRegKey HKCU "Software\Classes\SperoCoin"
